...
enum SelectedMainArenaTopOffset {
    User { offset: u64 },
...
struct RenderConfig {
    show_chunks: bool,
    show_tracker_notes: bool,
    show_explanations: bool,
    show_layout: bool,
    max_layout_chunks: usize,
    show_tcache_candidates: bool,
    max_tcache_chain: usize,
    max_fastbin_chain: usize,
    max_unsorted_chain: usize,
    max_regular_bins: usize,
    max_smallbin_chain: usize,
    max_largebin_chain: usize,
    show_tcache_struct_candidate: bool,
    show_main_arena_candidate: bool,
    main_arena_offset: Option<u64>,
    show_arena_experiment: bool,
    show_fastbin_experiment: bool,
    show_unsorted_experiment: bool,
    show_bin_experiment: bool,
    show_unsorted_bin: bool,
    show_fastbins: bool,
    show_regular_bins: bool,
    show_smallbins: bool,
    show_largebins: bool,
    show_heap_scan: bool,
    show_main_arena_top_candidate: bool,
    main_arena_top_offset: Option<u64>,
    ...
    allocator_views_preset: AllocatorViewsPreset,
    json: bool,
    json_out: Option<PathBuf>,
    live_tui: bool,
    break_conditions: Vec<AllocatorBreakCondition>,
}
...
struct CurrentOutputSink<'a> {
    config: &'a RenderConfig,
    json_writer: Option<&'a mut JsonWriter>,
}
...
pub struct LiveTuiSink<'a> {
    sender: mpsc::Sender<LiveTraceUpdate>,
    json_writer: Option<&'a mut JsonWriter>,
}
...
struct RecordingRelatedSink<'a> {
    inner: &'a mut dyn LiveTraceSink,
    related_records: Vec<json::JsonTraceRecord>,
}
...
fn write_live_update_json(writer: &mut JsonWriter, update: &LiveTraceUpdate) -> Result<()> {
    match update {
...
impl LiveTuiApp {
    pub fn apply_update(&mut self, update: LiveTraceUpdate) {
        match update {
            LiveTraceUpdate::SessionStart(session) => {
    ...
    fn selected_event_id(&self) -> Option<usize> {
    ...
    fn select_chunk_at_current_layout_index(&mut self) {
    ...
    fn update_selected_chunk_after_layout_update(&mut self) {
    ...
    fn selected_chunk_from_latest_layout(&self) -> Option<&json::JsonLayoutChunk> {
    ...
    fn select_chunk_index(&mut self, index: usize) {
    ...
    fn clear_selected_chunk(&mut self) {
    ...
    fn cancel_heap_search_prompt(&mut self) {
    ...
    fn execute_heap_search(&mut self) {
    ...
    fn find_chunk_in_latest_layout(&self, query: HeapSearchQuery) -> Option<usize> {
        let json::JsonTraceRecord::HeapLayout { chunks, .. } = self.latest_heap_layout.as_ref()?
        else {
            return None;
        };
...
fn replay_config_from_render_config(config: &RenderConfig) -> ReplayConfig {
    ReplayConfig {
...
impl ReplaySession {
    pub fn from_records(records: Vec<json::JsonTraceRecord>) -> Self {
    ...
    pub fn event_record(&self, selected_event_index: usize) -> Option<&json::JsonTraceRecord> {
        let event = self.events.get(selected_event_index)?;
        self.records.get(event.record_index)
    }

    pub fn records_for_event(&self, event_id: usize) -> Vec<&json::JsonTraceRecord> {
        self.records
            .iter()
