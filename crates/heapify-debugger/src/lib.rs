...
pub fn build_exec_plan(config: &LaunchConfig) -> Result<ExecPlan> {
    ...
    let effective_library_path = config.loader_path.as_ref().and_then(|_| {
        config
            .library_path
            .clone()
            .or_else(|| config.supplied_libc_path.as_ref().and_then(parent_dir))
    });
...
pub struct BreakpointManager {
    breakpoints: HashMap<u64, ManagedBreakpoint>,
}
...
pub fn read_unsorted_bin_experiment(
    ...
    for field_offset in (0x60..0x200u64).step_by(0x10) {
        ...
        let bk_offset = field_offset
            ...
            .with_context(|| format!("main_arena unsorted candidate bk offset overflow"))?;
...
pub fn read_unsorted_bin_snapshot(
    ...
    let bk_offset = field_offset
        .checked_add(profile.pointer_size)
        .with_context(|| format!("main_arena unsorted bk offset overflow"))?;
